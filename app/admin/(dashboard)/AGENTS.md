...
| File         | Description                                                          |
| ------------ | -------------------------------------------------------------------- |
| `layout.tsx` | Dashboard layout — sidebar navigation, mobile header, AI chat bubble |
...
- Each page directory contains: `page.tsx` (component) + `use-*-page.ts` (controller hook)
- Some pages have `_hooks/` or `_components/` for collocated private modules
- All pages share the sidebar layout from `layout.tsx`
